...
def faktorgraf(stdscr, COUNTER, show_marker):
    ...
    while True:
        ...
        for i in range(0, h-2): # høgda
            for j in range(0-user_offset, w-2, i+1): #bredda
                num = str(i+1) #det som printast
                if j >= 0:
                    if show_marker and j == w//2:
                        ...
                        stdscr.addstr(i+2, j, num, curses.color_pair(1))
                        if int(num) == 0:
                            stdscr.addstr(i+2, j, "_", curses.color_pair(1))
                    else:
                        stdscr.addstr(i+2, j, num)
                        
        #primtal P, kvadrattal K
        if show_marker:
...
def main(stdscr):
    ...
    while True:
        ...
        #if COUNTER > 0:
        #    stdscr.addstr(h-12, x_offs+12, "tau(n)="+str(tau(COUNTER)))
        ...
        if key == curses.KEY_RIGHT:
            ...
            show_marker = True
            if COUNTER > 4000:      #Fjern denne etter optimalisering
                show_marker = False
...
#primtal, limit, elap = erathostenes(10000)
primtal, primtal_limit, elap = tau_primes(100000)
